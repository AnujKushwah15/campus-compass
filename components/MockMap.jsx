    ...
    return (
        <div className="relative w-full h-[400px] md:h-[500px] bg-[#E5E0D5] rounded-2xl overflow-hidden border border-cc-purple-500 shadow-inner group animate-pop-in">
            ...
            <div className="absolute top-0 left-0 transition-all duration-300 ease-in-out hover:scale-110 cursor-pointer z-20 animate-bus-track">
                <div className="relative">
                    ...
                    <div className={`relative flex items-center justify-center w-12 h-12 rounded-full border-2 border-white shadow-lg text-white ${status === 'ontime' ? 'bg-cc-brown-600' : status === 'delayed' ? 'bg-amber-500' : 'bg-red-500'
                        ...
                        <div className="text-[10px] font-bold absolute -top-1 right-0 bg-white text-cc-purple-800 px-1 rounded-sm shadow-sm">
                            Bus 42
                        </div>
                    ...
                    {/* Tooltip on Hover */}
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max bg-white/90 backdrop-blur px-3 py-1 rounded-lg text-xs font-semibold text-cc-purple-800 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm pointer-events-none">
                        Arriving in 5 mins
                    </div>
                </div>
            </div>
            ...
            <div className="absolute top-[28%] left-[75%] z-10 flex flex-col items-center">
                <MapPin className="text-cc-purple-500 w-8 h-8 drop-shadow-md mb-1" fill="#8b5cf6" />
                <span className="bg-white/80 px-2 py-0.5 rounded text-[10px] font-bold text-cc-purple-800 shadow-sm">CAMPUS</span>
            </div>

            {/* Home/Start Marker */}
            <div className="absolute bottom-[20%] left-[10%] z-10 flex flex-col items-center">
                <div className="w-4 h-4 rounded-full bg-cc-purple-800 border-2 border-white shadow-md mb-1"></div>
                <span className="bg-white/80 px-2 py-0.5 rounded text-[10px] font-bold text-cc-purple-800 shadow-sm">START</span>
            </div>
        </div>
    );
}
